...
export const pywebviewBridge = {
    ...
    if (auth.currentUser && isFocus && activeSessionId) {
      try {
        ...
        const endResp = await fetch(
          `http://127.0.0.1:8000/v1/session/end?user_id=${auth.currentUser.uid}&xp_earned=${xpEarned}&session_id=${sid}&duration_minutes=${sessionDurationMinutes}`,
  ...
  getElevenLabsVoices: async (): Promise<{ name: string; id: string }[]> => {
    ...
    try {
      const response = await fetch('http://127.0.0.1:8000/v1/voices');
  ...
  playRoastStream: async (roastText: string, voiceId?: string): Promise<void> => {
    ...
    try {
      const response = await fetch('http://127.0.0.1:8000/v1/roast/stream', {
        method: 'POST',
